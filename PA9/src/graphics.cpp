...
void Graphics::Update(unsigned int dt, string motion[])
{
  ...
  controlUpdate( motion[1]);
  lights[0].LightPosition = glm::vec4(m_sphere->getPosition(), 1);
  m_camera->SetView( m_sphere->getPosition());
...
void Graphics::controlUpdate(string control)
{
    if(control == "I SPOT SIZE")
    {
        //lights[0].coneAngle += constant?
    }
    if(control == "D SPOT SIZE")
    {
        //lights[0].coneAngle -= constant?
    }
    if(control == "I SPOT BRIGHT")
    {
        lights[0].AmbientProduct += glm::vec4(1.0,1.0,1.0,0.0);
    }
    if(control == "D SPOT BRIGHT")
    {
        lights[0].AmbientProduct -= glm::vec4(1.0,1.0,1.0,0.0);
    }
    if(control == "I AMBIENT BRIGHT")
    {
        lights[1].AmbientProduct += glm::vec4(1.0,1.0,1.0,0.0);
    }
    if(control == "D AMBIENT BRIGHT")
    {
        lights[1].AmbientProduct -= glm::vec4(1.0,1.0,1.0,0.0);
    }
    if(control == "D SPEC")
    {
        //sphere/cube -> specularBrightness function
    }
    if(control == "I SPEC")
    {
        //sphere/cube -> specularBrightness function
