...
Top::Top( string fileName, btDiscreteDynamicsWorld* dynamicsWorld ) : Model( fileName )
   {
    // Create Cup collision shape
    btCollisionShape* fallShape = new btBoxShape( btVector3(0, 0, 0));
        
    // Create Cup motion state, place 50 meters above ground
    btDefaultMotionState* fallMotionState = new btDefaultMotionState( 
    btTransform( btQuaternion( 0, 0, 0, 1 ), btVector3( -75, 15.3, 70 ) ) );            
...
void Top::Update( btDiscreteDynamicsWorld* dynamicsWorld, unsigned int dt )
   {
    btTransform trans;
    btScalar m[ 16 ];
